...
func (c *Client) init(o *Options) error {
	...
	if len(o.User) > 0 {
		if len(a) != 2 {
			return fmt.Errorf("xmpp: invalid username (want user@domain): %s", o.User)
		}
		user = a[0]
		domain = a[1]
	...
	if o.User == "" && o.Password == "" {
		...
		if !foundAnonymous {
			return errors.New("ANONYMOUS authentication is not an option and username and password were not specified")
		}
	} else {
		...
		for _, m := range f.Mechanisms.Mechanism {
			if m == "X-OAUTH2" && o.OAuthToken != "" && o.OAuthScope != "" {
				...
				_, err := c.SendString(fmt.Sprintf(
					"<auth xmlns='%s' mechanism='X-OAUTH2' auth:service='oauth2' xmlns:auth='%s'>%s</auth>\n",
			...
			if m == "DIGEST-MD5" {
				...
				_, err := c.SendString(fmt.Sprintf(
					"<auth xmlns='%s' mechanism='DIGEST-MD5'/>\n",
					nsSASL,
				))
				if err != nil {
					return err
				...
				if err = c.p.DecodeElement(&ch, nil); err != nil {
					return fmt.Errorf("unmarshal <challenge>: %s", err.Error())
				}
	...
	default:
		return fmt.Errorf("expected <success> or <failure>, got <%s> in %s", name.Local, name.Space)
	}
	...
	if o.Resource == "" {
		_, err := c.SendString(fmt.Sprintf(
			"<iq type='set' id='%x'><bind xmlns='%s'></bind></iq>\n",
			cookie, nsBind,
		))
		if err != nil {
			return err
		...
		_, err := c.SendString(fmt.Sprintf(
			"<iq type='set' id='%x'><bind xmlns='%s'><resource>%s</resource></bind></iq>\n",
	...
	if err = c.p.DecodeElement(&iq, nil); err != nil {
		return fmt.Errorf("unmarshal <iq>: %s", err.Error())
	}
	if &iq.Bind == nil {
	...
	if o.Session {
		...
		_, err := c.SendString(fmt.Sprintf(
			"<iq to='%s' type='set' id='%x'><session xmlns='%s'/></iq>",
	...
	_, err = c.SendString(fmt.Sprintf(
		"<presence xml:lang='en'><show>%s</show><status>%s</status></presence>",
...
func (c *Client) startTLSIfRequired(f *streamFeatures, o *Options, domain string) (*streamFeatures, error) {
	...
	_, err = c.SendString("<starttls xmlns='urn:ietf:params:xml:ns:xmpp-tls'/>\n")
	if err != nil {
		return f, err
	}
	var k tlsProceed
	if err = c.p.DecodeElement(&k, nil); err != nil {
		return f, fmt.Errorf("unmarshal <proceed>: %s", err.Error())
	}
	...
	if err = t.Handshake(); err != nil {
		return f, fmt.Errorf("starttls handshake: %s", err.Error())
	}
	c.conn = t
...
func (c *Client) startStream(o *Options, domain string) (*streamFeatures, error) {
	...
	if err = c.p.DecodeElement(f, nil); err != nil {
		return f, fmt.Errorf("unmarshal <features>: %s", err.Error())
	}
	return f, nil
}
...
func (c *Client) Recv() (stanza interface{}, err error) {
	for {
		...
		// 	}
		// 	return Message{Type: "roster", Roster: r}, nil
		case *Presence:
			// return Presence{v.From, v.To, v.Type, v.Show, v.Status}, nil
			return *v, nil
		case *IQ:
			// TODO check more strictly
			if bytes.Equal(v.InnerElement.InnerXML, []byte(`<ping xmlns='urn:xmpp:ping'/>`)) || bytes.Equal(v.InnerElement.InnerXML, []byte(`<ping xmlns="urn:xmpp:ping"/>`)) {
...
func (c *Client) SendMessage(msg *Message) (n int, err error) {
	...
	// 	"<message to='%s' type='%s' xml:lang='en'><body>%s</body></message>",
	// 	xmlEscape(msg.To), xmlEscape(msg.Type), xmlEscape(msg.Body),
	// ))
	return c.SendElement(msg)

}

// SendString sends the original text without being wrapped in an XMPP message stanza.
// Do we need locks here?
func (c *Client) SendString(s string) (n int, err error) {
	// c.mu.Lock()
	// defer c.mu.Unlock()
	return fmt.Fprint(c.conn, s)
}
...
// SendElement sends stanza or any XML element
// Maybe rewrite using concrete interface? e.g. interface {GetID(), SetID()} ??
func (c *Client) SendElement(el interface{}) (n int, err error) {
	// TODO: check if id is empty and generate id if needed
	// TODO: add random generator
	// switch v := el.(type) {
	// case Message, Presence, IQ:
	// 	if v.ID == "" {
	// 		v.ID = "somerandomstring"
	// 	}
	// }
...
func (c *Client) SendHtml(msg Message) (n int, err error) {
	return c.SendString(fmt.Sprintf(
		`<message to='%s' type='%s' xml:lang='en'>
			<body>%s</body>
			<html xmlns='http://jabber.org/protocol/xhtml-im'><body xmlns='http://www.w3.org/1999/xhtml'>%s</body></html>
		</message>`,
		xmlEscape(msg.To), xmlEscape(msg.Type), xmlEscape(msg.Body), msg.Body,
	))
}

// Send stream error and close stream
func (c *Client) SendStreamError(e string) (n int, err error) {
	n, err = c.SendString(fmt.Sprintf(
		`<stream:error>%s</stream:error></stream:stream>`, e,
	))
	return
	// should we close connection as well ???
}

func (c *Client) SendIQ(iq *IQ) (n int, err error) {
	return c.SendElement(iq)
}
...
// Message is an incoming or outgoing XMPP chat message.
type Message struct {
...
func (m *Message) OtherStrings() []string {
	a := make([]string, len(m.OtherElements))
	for i, e := range m.OtherElements {
		a[i] = e.String()
	}
	return a
}
...
func (e *XMLElement) String() string {
	...
	// r := strings.NewReader(e.InnerXML)
	d := xml.NewDecoder(r)
...
func (q *Query) Parse() (stanza interface{}, err error) {
	...
	case NSRoster:
		r := Roster{}
		err = xml.Unmarshal(q.InnerXML, &r)
		return r, err
	}
...
func next(p *xml.Decoder) (xml.Name, interface{}, error) {
	...
	case nsClient + " error":
		nv = &Error{}
	default:
		return xml.Name{}, nil, fmt.Errorf("unexpected XMPP message %s <%s />", se.Name.Space, se.Name.Local)
	}
